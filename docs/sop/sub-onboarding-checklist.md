...
**Purpose:** Complete every item before dispatching the subcontractor regular work. 19 items total. Master onboarding flow that ties together vetting + trial + agreement + ServiceM8 setup + briefings.
...
1. ✅ Subcontractor completed [vetting-checklist](sub-vetting-checklist.md) (all mandatory items pass)
2. ✅ Subcontractor passed [trial-job-protocol](sub-trial-job-protocol.md) (80%+ rubric or conditional pass on 2nd trial)
...
- [ ] **1. Sign Subcontractor Agreement**
  ...
  - Store signed copy: Google Drive `Subcontractors/{SubName}/Agreement-Signed.pdf` + Xero supplier file
  - DocuSign automatically gives subcontractor their copy
...
- [ ] **2. Collect all onboarding documents**
  ...
  - Workers comp certificate (if subcontractor has any employees)
  - Driver's licence copy (signed subcontractor matches ABN holder)
  - Asbestos awareness training cert (SafeWork NSW)
  - Signed Agreement
  - **Store in**: `Subcontractors/{SubName-FullLegalName}/Documents/`
...
- [ ] **4. Add to Slack**
  ...
  - **Test**: send a message → confirm subcontractor receives + can reply
...
- [ ] **5. Share Quality Standards 1-pager**
  ...
  - Subcontractor keeps in their van
...
- [ ] **9. Set insurance expiry reminders**
  ...
  - **Standing rule**: subcontractor stood down immediately if insurance lapses without renewal

### Phase E — Briefings on critical clauses (steps 10-15)

Each briefing: walk subcontractor through plain-English version, get verbal acknowledgment, note date in their record.
...
- [ ] **19. Collect first NSW Subcontractor's Statement**
  ...
  - File in `Subcontractors/{SubName}/Statements/{date}.pdf`
  - **Standing rule**: no statement = no payment

- [ ] **20. Mock customer interaction (added by CEO 2026-05-01 PM)**
  - Marko role-plays a customer scenario with new subcontractor:
  ...
  - Subcontractor responds; Marko coaches if needed
  - Validates subcontractor can handle customer-facing moments before they're alone with our customers
...
Stored in `Subcontractors/{SubName}/Onboarding-Signoff.pdf`.
