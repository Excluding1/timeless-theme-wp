...
[COST ANCHOR]
"A full bathroom renovation typically costs $15,000 to $30,000.
...
**Why NPS first:** measures satisfaction and routes internal service recovery. The Google review request goes to EVERY customer regardless of score — no review-gating (ACCC-compliant per Decision 4 / Option A). Detractors additionally get an internal callback so we can make it right; their review invite is never suppressed.

### NPS Score Routing

- **9-10 (Promoter)** → tag `nps_promoter` → fire 4B Google Review Request → 7 days later fire 4D Referral
- **7-8 (Passive)** → tag `nps_passive` → fire 4B Google Review Request (no referral)
- **1-6 (Detractor)** → tag `nps_detractor` → 4C Internal Slack alert + Allan callback within 60min (service recovery) **AND** fire 4B Google Review Request — never suppressed (ACCC: no review-gating)

---

### 4B. Google Review Request (ALL customers — fires after NPS regardless of score)
...
We fixed one of your bathrooms previously, and if your other properties need attention, send us photos for a free 2-hour quote.
