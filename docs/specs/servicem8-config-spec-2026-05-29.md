...
> ⚠️ **FLAG — UNRESOLVED MODEL TENSION (surfaced by the 2026-06-05 repo-alignment; needs a Clifford+Cleo+field-ops re-lock before the sub agreement / app build).** This §6 (Staff/**SINGLE-account**, NOT Network) directly **contradicts** the 2026-06-05 docs (Override 16 / STATE / FUTURE-PLAN / OPERATING-CONTEXT / roles), which say *"subs via **SM8 Network** (own account, accept/decline)."* They can't both be canonical. **What the alignment exposed:** the 2026-06-05 **contractor-app blueprint** (`contractor-app-blueprint-2026-06-05.md`) is a **SINGLE-account** model — subs log into OUR app (Supabase Auth) → our ONE SM8 account; they do **not** hold their own SM8 Network accounts. That **vindicates THIS §6** (incl. its "custom contractor portal" = the app, line 104) and **undercuts the "Network" framing.** "SM8 Network" was the rec for an **app-less interim**; Allan's **app-first** decision makes the real model **single-account via the custom app** (with the app's own built-in accept/decline + Fair-Work guardrails — still contractor-safe). **Do NOT lock either as canonical until reconciled.** See `decision-sm8-keep-vs-build-2026-06-05.md`.
