...
> ✅ **RESOLVED — LOCKED 2026-06-05: single-account via the custom app** (NOT SM8 Network as canonical; NOT "subs-as-Staff"). Cleo + field-ops/Fair-Work + Clifford settled the tension: subs engage via the **Timeless single-account dispatch app** (own built-in, *logged* accept/decline). **This §6's *single-account* read is CORRECT** — its "custom contractor portal" (line 104) **is** this app. **BUT drop the "Staff" label below:** subs = **app users**, NOT SM8 staff (the Staff framing carries needless Fair-Work risk; SM8 Staff seats = Marko/Allan/employees only). **SM8 Network = app-less *interim* only** (not canonical, not a layered model). Full lock + Fair-Work rationale (app = low-weight tools factor · logged accept/decline beats Network's opaque one · NOT a Part-3A digital-labour-platform since Timeless is the principal not an intermediary) + binding guardrails + the canonical paragraph → `decision-sm8-keep-vs-build-2026-06-05.md` §SUB-ENGAGEMENT MODEL — LOCKED. The single-account evidence below stands; only the "Staff" wording is superseded.
