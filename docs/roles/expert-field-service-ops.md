...
- **ServiceM8 limitations**: limited custom field types, no native deep BigQuery sync (use webhooks), less flexible than Salesforce Field Service but fits trades 95% of the time. **No API for SM8 Network accept/decline** (UI/email-only) — which is exactly why the canonical model is OUR dispatch app with its **own** accept/decline state machine (the one genuinely-new build piece), not SM8 Network. Per decision doc §VIABILITY STUDY + §SUB-ENGAGEMENT MODEL — LOCKED.
- **Subs engaged via the Timeless single-account dispatch app, NOT SM8 Staff seats** (SUB-ENGAGEMENT MODEL LOCKED 2026-06-05): a sub authenticates to **OUR app** (Supabase Auth) → **OUR one SM8 account** (our backend = sole SM8 key-holder); the app's own built-in, **logged accept/decline = the right of refusal** (a job is OFFERED, never assigned → sub ACCEPTS or DECLINES → route to next). **Subs = app users, NOT SM8 "Staff"** — Staff seats are for Marko/Allan/genuine employees only. Subs supply own ABN + ≥$10M PL (verified before any offer). **SM8 Network (sub on their OWN SM8 account, Convert-to-Job/decline) is RETIRED as the canonical model — kept ONLY as an app-less interim/fallback (Path A)** if launching before the app ships. The accept/decline is the structural Fair-Work signal (the app's logged decline is a *stronger* evidence position than Network's opaque one) — see `auditor-fair-work.md` + `decision-sm8-keep-vs-build-2026-06-05.md` §SUB-ENGAGEMENT MODEL — LOCKED.
...
- **Reschedule pattern**: customer reschedules → **GHL/Twilio sends the new-time SMS to the customer** (not SM8) → the SM8 job date updates → subcontractor gets the notification **in the Timeless dispatch app** (canonical; or via SM8/Network on the interim/fallback path). Make this a 1-tap flow. (Sub-facing notifications come from the app/SM8; customer-facing SMS always from GHL/Twilio — no-contact rule.)
...
| Streamlined for ops | Dispatch is tag-based filter + 1-tap **offer** (accept/decline in the Timeless dispatch app; SM8 Network = interim/fallback) to right subcontractor |
...
- [decision-sm8-keep-vs-build-2026-06-05.md](../specs/decision-sm8-keep-vs-build-2026-06-05.md) — **canonical 2026-06-05**: keep SM8 (per-job/unlimited); **§SUB-ENGAGEMENT MODEL — LOCKED: subs engage via the Timeless single-account dispatch app (app's own logged accept/decline); SM8 Network = app-less interim/fallback only; subs = app users, not SM8 Staff**; no-customer-contact rule; never-penalise-a-decline
