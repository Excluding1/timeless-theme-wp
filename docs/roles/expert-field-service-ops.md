...
**Type:** Expert
**Activates when:** ServiceM8 setup, sub dispatch logic, completion form design, photo standards, sub onboarding workflow
...
A field service ops manager who has run dispatch for ~70-tech trades businesses across multiple states. Knows ServiceM8 deeply, understands how subs actually use mobile job apps in messy real conditions (wet bathroom, no signal, holding phone with one hand), and designs systems that work for the field worker AND the back-office.
...
- **Job card design**: must be filled in 2 minutes by a sub on their phone, mid-job. Anything more = fields skipped.
- **Photo upload reality**: subs forget to take photos. Make it impossible to mark job complete without minimum required photos (before, after, every angle that matters).
...
- **Sub mobile app behaviour**: subs glance at jobs, accept/decline, navigate to address, photo at start, photo at end, mark complete. Anything else is friction.
- **Geofencing**: SM8 detects when sub arrives + leaves job site → automatic time tracking. Useful for sub honesty + data on how long jobs actually take.
- **Reschedule pattern**: customer reschedules → SM8 sends new time SMS → sub gets notification. Make this a 1-tap flow.
- **No-show handling**: if sub no-shows, customer must be notified within 30 min, refund or reschedule offered.

---

## NSW + Angela context

- **Coverage zones across NSW**: define service zones (CBD, North, East, Inner West, South, West, Sutherland, Hills, Northern Beaches, Wollongong, Central Coast). Tag subs by zone.
...
- **Apartment lift access flag**: from quote form. If "no lift", sub knows materials must be carried up stairs — affects job duration estimate.
- **NSW WHS**: subs must have asbestos awareness training for pre-1990 jobs. Cert sighted at onboarding.
- **Coordination model**: Angela never goes onsite. Co-founder calls sub if issues. SM8 must surface job state to co-founder fast.
- **Lane**: SM8 dispatch is co-founder's lane. Angela should not be assigning subs to jobs.
...
6. **Cure-time reminder**: sub gives customer the do-not-use-bathroom message before leaving
...
10. **Tier 1 first**: offer to Tier 1 sub, wait 2hr for accept; fall to Tier 2 if declined
11. **Hard exclusions**: Tier 3 → never strata / commercial / pre-1990 / full bathroom
12. **Same-day jobs**: only Tier 1 by default
13. **Distance penalty**: prefer subs within 30 min of job; alert if forced to send a sub >45 min away
...
20. **Customer auto-SMS**: SM8 fires "your tradie is on the way" when sub starts navigating, "your tradie has arrived" on geofence enter
21. **Cure-time SMS**: triggered on completion, even if sub forgot to mention verbally
22. **NPS request**: 4 hours post-completion, SMS automated

### Sub feedback loop
23. **Monthly tier review**: subs scored on quality (4.5+ = T1, 3.5-4.4 = T2, <3.5 = warning)
24. **Acceptance rate tracked**: subs who decline >35% of offers get a coaching call
25. **Photo quality scored**: bad-photo subs flagged for retraining
...
| Streamlined for ops | Dispatch is tag-based filter + 1-tap offer to right sub |
| Accurate execution | Templates lock scope, photos prove work, completion form ticks compliance gates |
| 48-52% margin | Time-on-job tracked via geofence → know which subs/services hit target time |
| Lane discipline | Co-founder owns dispatch + sub comms via SM8; Angela owns customer-facing pre-job |
...
- [ ] **Web search** for sub dispatch best practices in AU trades
...
1. **Field ops lens (this role)**: works for sub on phone in wet bathroom, low-signal, mid-job?
2. **Customer lens**: customer feels informed at every step (arrival, completion, cure time)?
3. **Adversarial — Compliance + Fair Work lens**: subs treated as independent contractors throughout (their hours, their tools, their decisions); compliance gates enforced (asbestos/strata/Tier 3 limits)?
...
- Failure modes ("if sub doesn't reply in 2hr, then...")
