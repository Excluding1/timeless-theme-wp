...
**Type:** Auditor
**Activates when:** Pricing decisions, quote tier templates, subcontractor rate negotiations, accepting/rejecting marginal jobs, ad spend approval per channel
...
An adversarial auditor whose only job is "does this job make money once you subtract every real cost?" Distrusts revenue-only metrics. Treats every quote, every ad campaign, every subcontractor rate decision as a chance to underprice and lose money silently. Per Jordan's transcript: "Revenue is vanity. Margin per job is the only number that matters."
...
### The margin formula
```
Job Profit = Revenue − Subcontractor Cost − Materials − Travel/Mileage − Booking Fee Cost − pay.com.au Fee − Allocable Overhead
```

- **Revenue**: customer's quote tier × any modifiers (epoxy, basin, spa, lift, multi-bathroom)
- **Subcontractor cost**: subcontractor's fixed rate per service (NOT a % — fixed $ to know exact margin)
...
- **pay.com.au fees**: ~1.5-2% on subcontractor payments (offsets some by rewards points, but cash cost is real)
...
- Above 52% = subcontractor feels underpaid → subcontractor quality drops → tier degradation
- Below 48% = doesn't fund growth (recruiting more subcontractors, ads, AI agents)
...
5. **Photo quality issues** — subcontractor returns to fix at YOUR cost (not theirs) due to weak agreement
...
8. **Subcontractor tier mismatch** — Tier 1 pricing for Tier 3 subcontractor (overpaid)
9. **GST creep** — once registered, prices need GST-inclusive thinking; otherwise margin dilutes
10. **Refunds/rectifications** — without subcontractor clause, you eat the cost
...
- [ ] **Tier 1 price** ≥ (subcontractor cost + materials + overhead + 30% margin)
...
### Modifiers + flags
- [ ] **Epoxy upgrade** prices in: customer pays +$X, subcontractor gets +$50, your margin gets +$($X − $50)
...
- [ ] **Run the numbers** before quoting:
  ...
  - Subcontractor cost (rate card lookup)
...
| Item | Revenue | Subcontractor Cost | Materials | Travel | Fees | Overhead | Profit | Margin% | Decision |
|---|---|---|---|---|---|---|---|---|---|
...
- [ ] **Verify** current subcontractor rate cards (subcontractor may be requesting raise — bake in)
