...
Fully local & offline on macOS via Apple's Vision framework (no cloud, no torch):
 - on-screen TEXT via VNRecognizeTextRequest (burned-in captions / overlays)
...
def _keep_ocr(s):
    ...
    Short-video auto-captions flash one blurry word at a time — those duplicate the
...
def analyse(media_path, subdir, vid, progress_cb=None):
    """Return (visual_track_text, rel_path). `subdir` = "<platform>/<username>"."""
    if not available():
