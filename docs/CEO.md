...
> **SM8-KEEP DECISION 2026-06-05 (LOCKED, unanimous: field-service-ops expert + Cleo + Clifford):** ServiceM8 is **KEPT as the field-service backbone — not replaced by a custom app.** Triggered by Allan's *"do we even need ServiceM8 if we just make our own app?"* A custom **contractor-VIEW** app is APPROVED but is a VIEW on the same SM8+GHL+Sheet data (zero migration), scheduled LAST. *(UPDATE 2026-06-08: app PULLED FORWARD → now 🔨 BUILDING — "scheduled LAST" superseded; v1 LOCKED + schema scaffolded in `contractor-app/`. STATE.md §6.)* Subs engaged via the **Timeless single-account dispatch app** (subs → OUR app → OUR one SM8 account; the app's own built-in, logged accept/decline = right of refusal; own ABN + ≥$10M PL gate). **Subs = app users, NOT SM8 Staff** (Staff seats only for Marko/Allan/genuine employees). **SUB-ENGAGEMENT MODEL LOCKED 2026-06-05 — SM8 Network (subs on own accounts) is RETIRED as canonical, kept only as an app-less interim/fallback.** See [`docs/specs/decision-sm8-keep-vs-build-2026-06-05.md` §SUB-ENGAGEMENT MODEL — LOCKED](specs/decision-sm8-keep-vs-build-2026-06-05.md). **COST CORRECTION:** SM8 = per-JOB, flat monthly, UNLIMITED users — the "per-staff/per-seat" assumption (incl. the spreadsheet rationale below) is STALE.
...
### Override 16 (2026-06-05): KEEP ServiceM8 as the backbone · custom contractor-VIEW app APPROVED — ~~scheduled LAST~~ **PULLED FORWARD → BUILDING (2026-06-08)** · subs get NAME + ADDRESS only
...
**2. Custom contractor-VIEW app = APPROVED, scheduled LAST.** *(UPDATE 2026-06-08: PULLED FORWARD — now 🔨 BUILDING; "scheduled LAST" superseded — v1 LOCKED + schema scaffolded in `contractor-app/`. STATE.md §6.)* It is a **VIEW on the same SM8 + GHL + Sheet data — ZERO migration**, NOT an SM8 replacement. Adding it later = subs switch *screens*, not systems; nothing migrates. The permanent / hard-to-redo part is the **data + rules** (incl. the no-contact rule) — nail those now. Architecture (forced by two hard realities: SM8 key = full-account / no per-sub scoping, and accept/decline has NO API): **sub-app → OUR backend (Supabase BFF/token-broker; sole SM8 key-holder; per-sub JWT+RLS; contact-field filter; rate governor) → SM8.** Subs never hold the key. The one genuinely-new build piece = a custom **accept/decline state machine** (SM8 Network's native accept/decline is UI/email-only). Full stack/phase plan = the blueprint doc.

**3. Subs engaged via the Timeless single-account dispatch app (SUB-ENGAGEMENT MODEL LOCKED 2026-06-05 — supersedes the "SM8 Network" framing this item previously carried).** Subs authenticate to **OUR app** (Supabase Auth) → **OUR one SM8 account** (our backend = sole SM8 key-holder); the app's own built-in, **logged accept/decline = the right of refusal**. Subs are **app users, NOT SM8 "Staff"** (SM8 Staff seats = Marko/Allan/genuine employees only). **SM8 Network (subs on their own accounts) is RETIRED as the canonical model** — kept ONLY as a documented **app-less interim/fallback (Path A)** if we ever launch before the app ships; NOT a layered model under the app. Either path: own ABN + ≥$10M PL before any offer; sub sees only their job (no pricing/margin/other customers); **they invoice us** per job; SM8 is per-JOB / unlimited users so subs cost nothing extra. **Canonical statement + binding guardrails: [`decision-sm8-keep-vs-build-2026-06-05.md` §SUB-ENGAGEMENT MODEL — LOCKED](specs/decision-sm8-keep-vs-build-2026-06-05.md). ⚖️ Legal review of the sub agreement required before the first sub signs.**
...
**5. FAIR WORK guardrail (s15AA whole-of-relationship test + Mar-2026 ATO/FWO construction clampdown).** The app's accept/decline + own-ABN = genuine-contractor signal; the app is a **low-weight "tools" factor** (sub supplies the high-cost grinder/coatings/vehicle) and is **NOT a Part-3A digital-labour-platform** (Timeless is the principal/customer, not an intermediary). The app's logged decline is a **stronger** evidence position than Network's opaque one. Staff allocation (no right of refusal) = employee-leaning → sham-contracting risk (up to **$99k/contravention**) + super back-pay. **NEVER penalise a sub's decline** — any "decline >X% → coaching/downgrade" idea is reframed as a **capacity/fit conversation off-system**, never a punishment surfaced to the sub or coded into the app (penalising refusal is an employee indicator). Keep: offer-language not orders · NO auto-assign (offer → decline → re-route, never re-assign) · real right to delegate · own-ABN + ≥$10M PL gate before offers · no `@company` email / uniform / "our team" branding for subs · they invoice us · watch >80%-revenue exclusivity (PSI). **(Guardrails now apply to the app — see [decision doc §SUB-ENGAGEMENT MODEL — LOCKED](specs/decision-sm8-keep-vs-build-2026-06-05.md).)**

**6. SEQUENCING (Allan 2026-06-05): internal backbone FIRST → app → launch.** *(UPDATE 2026-06-08: app PULLED FORWARD — Phase 1 foundation now building in parallel with the backbone, per STATE.md §6.)* Internal backbone = finish Make scenarios (1 + strip-contact + find-or-create, then 2–5) + publish/finish GHL workflows + SM8 config (Marko as staff) + Slack alerts + deploy the GHL-wired quote form + end-to-end test. THEN build the contractor app. THEN run real jobs. *(Phase 1–3 of the app — Make strip-contact + read path — is path-independent and can be done as part of the internal backbone now.)*
...
| **Total monthly with ads** | ~~**~$38.60**~~ **~$193.60** *(base corrected 2026-06-08 to match row above; with-ads columns still computed off the old $38.60 base — **recompute**)* | **~$478-778** | **~$1,708** |
...
| 2026-06-05 | **Custom contractor-VIEW app APPROVED but scheduled LAST** (after internal backbone, before launch). It's a VIEW on the same SM8+GHL+Sheet data — ZERO migration; one new build piece = custom accept/decline state machine. *(UPDATE 2026-06-08: PULLED FORWARD — now 🔨 BUILDING; "scheduled LAST" superseded; v1 LOCKED + schema scaffolded in `contractor-app/`. STATE.md §6.)* | Allan wants it permanent + fears a messy migration; reframe — adding it later = subs switch screens, not systems. Blueprint ready. | Allan call; CEO + AI team build |
| 2026-06-05 | ~~**Subs engaged via SM8 Network (own account, accept/decline); staff seats only for Marko/Allan/genuine employees.**~~ **SUPERSEDED same day by the SUB-ENGAGEMENT MODEL lock below** — Network demoted to app-less interim/fallback. Own ABN + ≥$10M PL gate before offers (unchanged). | Genuine-contractor signal (s15AA whole-of-relationship test + Mar-2026 ATO/FWO clampdown); avoids sham-contracting (up to $99k/contravention). | Both founders + CEO |
| 2026-06-05 | **SUB-ENGAGEMENT MODEL LOCKED — subs engage via the Timeless single-account dispatch app** (subs → OUR app → OUR one SM8 account; the app's own built-in, logged accept/decline = the right of refusal). **Subs = app users, NOT SM8 Staff. SM8 Network (subs on own accounts) RETIRED as the canonical model** — kept only as an app-less interim/fallback (Path A). Own ABN + ≥$10M PL gate before offers. ⚖️ Legal review of the sub agreement before the first sub signs. See [decision doc §SUB-ENGAGEMENT MODEL — LOCKED](specs/decision-sm8-keep-vs-build-2026-06-05.md). | App's logged decline = stronger Fair-Work evidence than Network's opaque one; app = low-weight tools factor, NOT a Part-3A digital-labour-platform (Timeless is the principal, not an intermediary); resolves the §6-vs-06-05 tension. | Allan app-first call; unanimous Cleo + field-ops/Fair-Work + Clifford |
| 2026-06-05 | **NO-CUSTOMER-CONTACT rule LOCKED: subs get NAME + ADDRESS only, never customer phone/email.** Customer comms move to GHL/Twilio; Make Scenario 1 drops `Add Job Contact` + strips contact from the SM8 job. Supersedes "SM8 auto-SMS off Job Contact". | Anti-poaching — protect the customer relationship (#1 asset); cleaner for the sub. | Allan + CEO; **Make change ✅ DONE + verified live 2026-06-07** (`Add Job Contact` dropped; SM8 jobs show name + address only; job #14 passed — STATE.md §6). |
...
4. **Our margin is EARNED by being the demand-and-management engine — pay subs WHOLESALE, not retail** (core unit-economics, locked 2026-06-18). Retail sub quotes ($980 for a strip-back bath, $2,400/room tile) bake in the sub's OWN marketing/CAC, quoting, chasing, admin + risk. We strip ALL of that out — we run the ads, own the customer + every message/follow-up, quote, take the deposit, schedule, carry the brand + risk; the sub *just books a time and does the job*. That leverage earns us a **wholesale rate (≈ the master sheet's cost line)**, and a sub trading a lower per-job rate for **zero CAC + zero admin + consistent volume can earn MORE per actual worked hour** — genuine win-win. The sheet's ~44-55% is real **once the demand engine is live AND partner subs are signed on wholesale terms**; until then (low volume, ads not at scale, no signed subs, legal gate) early jobs run thinner at retail-ish rates and margin climbs as the leverage comes online. **Implication: the GHL pipeline / ads / follow-ups / contractor app are not overhead — they ARE the margin machine.** Don't price as if we're a retail customer to the sub; price as the operator who removes the sub's hardest, most expensive work.
