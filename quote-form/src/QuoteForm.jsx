...
const SVCS = {
  ...
  bath: [
    ...
    { id: "bt4", label: "Fix the mouldy seal", trade: "silicone replacement", tip: "Replace silicone around bath rim — stops leaks and removes mould. (Already included if you've ticked Resurface above — no need to tick both.)", befImg: "https://placehold.co/200x140/3a2e22/fff?text=Before", aftImg: "https://placehold.co/200x140/f0eeea/333?text=After", befTxt: "Peeling silicone", aftTxt: "Fresh sealed edge" },
  ],
  basin: [
...
function Trust() {
  // Claims must be currently verifiable. "NSW Licensed" removed until builder
  // licence is issued (Fair Trading exposure). Warranty is "Up to 5yr" — varies
  // by service tier (some services carry shorter warranty per pricing schedule).
...
export default function QuoteForm() {
  ...
  // ─── Photos-step "quick details" gating ────────────────────────────
  // Two questions on the photos step have *different* trigger conditions:
  //   - prevResurfaced: only matters for jobs that LAY a coat over existing
  //     surface (resurface) — strip-back time is priced separately.
  //   - ventilation: matters for ANY chemical-fume work — resurface AND
  //     regrout (per Excel rejection #19). Previously gated only to resurface,
  //     missing the regrout case where epoxy/grout-removal solvents need airflow.
  const _sp = (a) => svcPicks[a] || [];
  const isResurfaceJob =
    selAreas.includes("bath") || activeAreas.includes("bath") ||
    _sp("shower").includes("sh4") || _sp("walls").includes("wl2") ||
    _sp("floor").includes("fl2") || _sp("vanity").includes("vn1") ||
    _sp("vanity").includes("vn3") || _sp("basin").includes("bs1") ||
    multiPicks.includes("m_bath_resurface") || multiPicks.includes("m_shower_recoat") ||
    multiPicks.includes("m_basin") || multiPicks.includes("m_full");
  const isRegroutJob =
    _sp("shower").some(id => ["sh1", "sh2"].includes(id)) ||
    _sp("floor").includes("fl1") ||
    _sp("walls").includes("wl1") ||
    _sp("vanity").includes("vn2") ||
    multiPicks.some(id => ["m_shower_regrout", "m_floor", "m_walls", "m_regrout_all"].includes(id));
  const needsPrevResurfacedQ = isResurfaceJob;
  const needsVentilationQ = isResurfaceJob || isRegroutJob;
  const needsQuickDetails = needsPrevResurfacedQ || needsVentilationQ;
                ...
                {!waitlistSent ? (
                  <>
                    ...
                    <button
                      ...
                      disabled={!emOk || !fnOk}
                      style={{ padding: "10px 14px", borderRadius: 8, border: "none", background: emOk && fnOk ? C.pri : C.brd, color: C.white, fontSize: 12, fontWeight: 600, cursor: emOk && fnOk ? "pointer" : "not-allowed", fontFamily: "inherit" }}
                    >
      ...
      {step === "photos" && <>
        ...
        {needsQuickDetails && (
          <div style={{ marginBottom: 16, padding: 14, background: C.surfLow, borderRadius: 12, border: `1px solid ${C.brd}` }}>
            ...
            {/* Previously resurfaced — only for jobs that lay a coat over existing surface */}
            {needsPrevResurfacedQ && (
            ...
            {/* Ventilation — shows for ANY chemical work: resurface (fumes from
                spray coatings) AND regrout (grout-removal solvents + epoxy fumes).
                Required by Excel rejection #19 across all such job types. */}
            {needsVentilationQ && (
              <div>
