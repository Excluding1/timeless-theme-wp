...
function Back({ onClick }) { return <button onClick={onClick} aria-label="Go back to the previous step" style={{ fontSize: 14, color: C.pri, background: "none", border: "none", cursor: "pointer", fontWeight: 500, marginBottom: 12, padding: "10px 8px", marginLeft: -8, minHeight: 44, display: "inline-flex", alignItems: "center" }}>← Back</button>; }
function OptGrid({ opts, val, set, cols = 3, label }) {
  return <div role="group" aria-label={label || "Selection"} style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 8 }}>{opts.map(o => <button key={o.id} type="button" onClick={() => set(o.id)} aria-pressed={val === o.id} style={{ padding: "12px 6px", borderRadius: 10, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", border: val === o.id ? `2px solid ${C.pri}` : `1.5px solid ${C.brd}`, background: val === o.id ? `${C.pri}08` : C.white, transition: "all 0.15s", minHeight: 44 }}><div style={{ marginBottom: 4, display: "flex", justifyContent: "center" }}>{o.icon}</div><div style={{ fontSize: 12, fontWeight: 500, color: val === o.id ? C.pri : C.sec }}>{o.label}</div></button>)}</div>;
}
      ...
      {step === "about" && <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div><label style={{ fontSize: 14, fontWeight: 600, color: C.pri, display: "block", marginBottom: 6 }}>Last name *</label><input type="text" value={ln} onChange={e => setLn(e.target.value)} placeholder="Last name" autoComplete="family-name" style={{ width: "100%", padding: "13px 14px", borderRadius: 10, border: `1.5px solid ${C.brd}`, fontSize: 16, fontFamily: "inherit", boxSizing: "border-box" }} /></div>
          </div>
          {!noPhone ? (
            <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              style={{ width: "100%", padding: "13px 14px", borderRadius: 10, border: `1.5px solid ${addrOk === false ? C.err : C.brd}`, fontSize: 16, fontFamily: "inherit", boxSizing: "border-box" }}
            />
      ...
      {step === "what" && <>
        ...
        <div onClick={toggleFullBathroom} role="button" tabIndex={0} aria-pressed={fullBathroomMode} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleFullBathroom(); } }} style={{ background: C.white, border: fullBathroomMode ? `2px solid ${C.pri}` : `2px solid ${C.acc}80`, borderRadius: 12, cursor: "pointer", overflow: "hidden", marginBottom: 14, transition: "all 0.15s", position: "relative" }}>
          ...
          <div style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, borderTop: `1px solid ${C.brd}` }}>
            ...
            {I.sparkle(22)}
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: C.pri }}>Full bathroom makeover</div>
                ...
                <div key={areaId}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "0 0 6px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 4px", whiteSpace: "nowrap" }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 700, color: C.pri, letterSpacing: "-0.01em" }}>{areaCfg.label}</div>
                    </div>
                    <div style={{ flex: 1, height: 1, background: C.brd }} />
                  </div>
            ...
            {getFullBathroomSections(fullBathroomInventory, fullAreaServices, !!chipRepairAddon.full).map(section => (
              <div key={section.id} style={{ padding: 14, border: `1.5px solid ${C.brd}`, borderRadius: 12, background: C.white }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, color: C.pri }}>{section.label}</div>
                </div>
                ...
                <div key={areaId} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ padding: 14, border: `1.5px solid ${C.brd}`, borderRadius: 12, background: C.white }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 700, color: C.pri }}>{areaCfg.label} photos</div>
                    </div>
