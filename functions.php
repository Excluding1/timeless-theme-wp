...
function timeless_webp_picture_filter( $html ) {
    ...
    return preg_replace_callback(
        // Match <img ... src="X.jpg|jpeg|png" ...>  (single or double quotes)
        '/<img\b(?<attrs_before>[^>]*?)\bsrc=(?<q>["\'])(?<src>[^"\']+\.(?:jpe?g|png))\k<q>(?<attrs_after>[^>]*)>/i',
        function ( $m ) use ( $template_dir, $template_url ) {
            $img_tag = $m[0];
            $src     = $m['src'];

            // Don't process external URLs or data URIs — only local theme assets
            if ( strpos( $src, 'data:' ) === 0 ) return $img_tag;
            if ( preg_match( '#^https?://#', $src ) && strpos( $src, $template_url ) !== 0 ) {
                return $img_tag;
            }

            // Convert URL to filesystem path, check companion .webp exists
            $local_path = $template_dir . '/' . ltrim( str_replace( $template_url, '', $src ), '/' );
            $webp_path  = $local_path . '.webp';
            if ( ! file_exists( $webp_path ) ) {
                return $img_tag; // No companion — serve original unchanged
            }

            // Build the picture element. The <source> srcset is the WebP URL.
            // The original <img> tag is preserved as fallback (last child of <picture>).
            $webp_url = $src . '.webp';
