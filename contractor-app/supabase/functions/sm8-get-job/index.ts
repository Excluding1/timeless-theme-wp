...
import { sm8JobToMirror, assertNoContact } from '../_shared/contactFilter.ts';
...
Deno.serve(async (req) => {
  ...
  if (!ADMIN_TOKEN || !secureCompare(token, ADMIN_TOKEN)) return fail(401, 'unauthorized', origin);
  ...
  try {
    ...
    try { assertNoContact(mirror); } catch { return fail(502, 'sm8_filter_failed', origin); } // fail-closed
