...
import { isValidUuid, updateJob } from '../_shared/sm8Client.ts';
...
const BATCH = 50;
const WRITABLE_KINDS = new Set(['status']); // 'queue_uuid' lands with the deferred accept-writeback
...
Deno.serve(async (req) => {
  ...
  for (const row of due ?? []) {
    ...
    try {
      await updateJob(jobUuid, { [kind]: String(row.target_value) });
